...
const QREyes = ({ x = -1, y = -1, fillColor, size, }) => (<Svg x={x} y={y}>
    <G transform={`scale(${size / SVG_SIZE})`} x={x} y={y}>
      <Path clipRule="evenodd" d="M0 12C0 5.37258 5.37258 0 12 0H28C34.6274 0 40 5.37258 40 12V28C40 34.6274 34.6274 40 28 40H12C5.37258 40 0 34.6274 0 28V12ZM28 6.27451H12C8.8379 6.27451 6.27451 8.8379 6.27451 12V28C6.27451 31.1621 8.8379 33.7255 12 33.7255H28C31.1621 33.7255 33.7255 31.1621 33.7255 28V12C33.7255 8.8379 31.1621 6.27451 28 6.27451Z" fill={fillColor} fillRule="evenodd"/>
      <Path d="M11 17C11 13.6863 13.6863 11 17 11H23C26.3137 11 29 13.6863 29 17V23C29 26.3137 26.3137 29 23 29H17C13.6863 29 11 26.3137 11 23V17Z" fill={fillColor}/>
    </G>
  </Svg>);
const QREyeBG = ({ x = -1, y = -1, size, backgroundColor, }) => (<Svg x={x} y={y}>
    <G transform={`scale(${size / SVG_SIZE})`} x={x} y={y}>
 ...
 */
export function QRCode({ value, size, color, backgroundColor: inputBackgroundColor, overlayColor = '#FFFFFF', quietZone = 8, ecl = 'H', }) {
    ...
    const backgroundColor = inputBackgroundColor !== null && inputBackgroundColor !== void 0 ? inputBackgroundColor : colors.surface1.val;
    const cornerPosition = (size - eyeSize) / EYE_OFFSET_MULTIPLIER;
    return (<Svg height={size} viewBox={[-quietZone, -quietZone, size + quietZone * 2, size + quietZone * 2].join(' ')} width={size}>
      <Defs>
      ...
      <G>
        <Rect fill={backgroundColor} height={size + quietZone * 2} rx={24} width={size + quietZone * 2} x={-quietZone} y={-quietZone}/>
      </G>
      <G>
        <Path d={path} fill={color}/>
        <Path d={path} fill={overlayColor + '2D'}/>
        <QREyeWrapper backgroundColor={backgroundColor} fillColor={color} overlayColor={overlayColor + '2D'} size={eyeSize}/>
        <QREyeWrapper backgroundColor={backgroundColor} fillColor={color} overlayColor={overlayColor + '2D'} size={eyeSize} y={cornerPosition}/>
        <QREyeWrapper backgroundColor={backgroundColor} fillColor={color} overlayColor={overlayColor + '2D'} size={eyeSize} x={cornerPosition}/>
      </G>
    </Svg>);
}
