 ...
 */
export function SegmentedControl({ options, selectedOption, onSelectOption, size = 'default', disabled, fullWidth, outlined = true, }) {
    ...
    return (<Tabs unstyled activationMode="manual" orientation="horizontal" value={selectedOption} onValueChange={(option) => {
            onSelectOption(option);
        }}>
      <OptionsSelector disablePassBorderRadius unstyled backgroundColor="transparent" fullWidth={fullWidth} outlined={outlined} loop={false} size={size}>
        {options.map((option, index) => {
            const { value, display, wrapper } = option;
            const optionButton = (<OptionButton key={value} active={selectedOption === value} disabled={disabled} fullWidth={fullWidth} size={size} value={value} onInteraction={handleOnInteraction} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(undefined)} onPress={() => {
                    onSelectOption(value);
                }}>
              {display !== null && display !== void 0 ? display : (<Text color={getOptionTextColor(selectedOption === value, hoveredIndex === index, disabled)} userSelect="none" variant={size === 'large' ? 'buttonLabel3' : 'buttonLabel4'}>
                  {value}
                </Text>)}
            </OptionButton>);
            if (wrapper) {
        ...
        <AnimatePresence>
          {activeAt && (<TabsRovingIndicator height={activeAt.height} width={activeAt.width} x={activeAt.x - TOGGLE_PADDING + activeIndicatorXAdjustment} y={activeAt.y - TOGGLE_PADDING + activeIndicatorYAdjustment - (!outlined ? 1 : 0)}/>)}
        </AnimatePresence>
      </OptionsSelector>
    </Tabs>);
}
...
//# sourceMappingURL=SegmentedControl.jsx.map