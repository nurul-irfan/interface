...
import { AnimatePresence, RadioGroup } from 'tamagui';
import { Flex } from 'ui/src/components/layout';
// Used to pass the selected value of the RadioGroup down to the RadioButtons.
const RadioButtonGroupContext = createContext(undefined);
/**
...
export function RadioButtonGroup(props) {
    ...
    return (<RadioButtonGroupContext.Provider value={selectedValue}>
      <RadioGroup {...props} flexDirection={props.orientation === 'horizontal' ? 'row' : 'column'} onValueChange={(selected) => {
            var _a;
            (_a = props.onValueChange) === null || _a === void 0 ? void 0 : _a.call(props, selected);
 ...
 * @returns
 */
export function RadioButton({ value, variant = 'default', ...rest }) {
    ...
    return (<RadioGroup.Item {...rest} unstyled alignItems="center" animation="simple" backgroundColor="transparent" borderColor={isSelected ? accentColor : '$neutral2'} borderRadius="$roundedFull" borderWidth="$spacing2" cursor="pointer" disabledStyle={{
            borderColor: '$neutral3',
        }} height={sizes.RadioButton} hoverStyle={{
            borderColor: isSelected ? accentColor : '$neutral2',
        }} id={id} justifyContent="center" value={value} width={sizes.RadioButton} onBlur={() => setIsFocused(false)} onFocus={() => setIsFocused(true)} onMouseDown={() => setIsPressed(true)} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onMouseUp={() => setIsPressed(false)}>
      {/* RadioGroup.Indicator is the inner dot which is shown when the item is selected. */}
      <RadioGroup.Indicator unstyled animation="simple" backgroundColor={rest.disabled ? '$neutral3' : accentColor} borderRadius="$roundedFull" height={indicatorSize} width={indicatorSize}/>
      {/* This is an inner dot shown in in *unselected* hovered/focused states. */}
      {!isSelected && (<AnimatePresence initial>
          {isHovered && !rest.disabled && (<Flex key={`UnselectedHoverIndicator-${value}`} animation="simple" backgroundColor="$neutral2" borderRadius="$roundedFull" enterStyle={{ scale: 0 }} exitStyle={{ scale: 0 }} height={unselectedHoverIndicatorSize} position="absolute" width={unselectedHoverIndicatorSize}/>)}
        </AnimatePresence>)}
      {/* This outer ring is only shown when the button is focused. */}
      <Flex alignItems="center" animation="simple" borderColor={getFocusedRingColor(variant, isFocused, isSelected, accentColor)} borderRadius="$roundedFull" borderWidth="$spacing1" height={sizes.FocusRing} justifyContent="center" position="absolute" width={sizes.FocusRing}/>
    </RadioGroup.Item>);
}
