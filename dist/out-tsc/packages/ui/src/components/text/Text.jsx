...
export const TextPlaceholder = ({ children }) => {
    return (<Flex row alignItems="center" testID="text-placeholder">
      <Flex row alignItems="center" position="relative">
        <HiddenFromScreenReaders>{children}</HiddenFromScreenReaders>
        <Flex backgroundColor={isWeb ? '$surface3' : '$surface2'} borderRadius="$roundedFull" bottom="5%" left={0} position="absolute" right={0} top="5%"/>
      </Flex>
    </Flex>);
};
export const TextLoaderWrapper = ({ children, loadingShimmer, }) => {
 ...
 */
export const Text = TextFrame.styleable(({ loading = false, allowFontScaling, loadingPlaceholderText = '000.00', ...rest }, ref) => {
    const enableFontScaling = useEnableFontScaling(allowFontScaling);
    if (loading) {
        return (<TextLoaderWrapper loadingShimmer={loading !== 'no-shimmer'}>
