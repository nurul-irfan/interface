...
describe(useDynamicFontSizing, () => {
    it('returns maxFontSize if text input element width is not set', () => {
        const { result } = renderHook(() => useDynamicFontSizing(MAX_CHAR_PIXEL_WIDTH, MAX_INPUT_FONT_SIZE, MIN_INPUT_FONT_SIZE));
    ...
    it('returns maxFontSize as fontSize if text fits in the container', async () => {
        const { result } = renderHook(() => useDynamicFontSizing(MAX_CHAR_PIXEL_WIDTH, MAX_INPUT_FONT_SIZE, MIN_INPUT_FONT_SIZE));
        await act(() => {
            result.current.onLayout({ nativeEvent: { layout: { width: 100 } } });
...
//# sourceMappingURL=useDynamicFontSizing.test.js.map