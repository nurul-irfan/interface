...
export function Shine({ children, disabled }) {
    ...
    if (!layout) {
        return (<Flex opacity={0} onLayout={(event) => {
                setLayout(event.nativeEvent.layout);
            }}>
        {children}
      </Flex>);
    }
    return (<MaskedView maskElement={children} style={{ width: layout.width, height: layout.height }}>
      ...
      <Reanimated.View style={[StyleSheet.absoluteFill, animatedStyle]}>
        <LinearGradient colors={[
            ...
            opacify(0, colors.neutral2.val.slice(0, 7)),
        ]} end={{ x: 1, y: 0 }} start={{ x: 0, y: 0 }} style={StyleSheet.absoluteFill}/>
      </Reanimated.View>
    </MaskedView>);
}
