...
describe(CurrencyLogo, () => {
    it('renders without error', () => {
        const tree = render(_jsx(CurrencyLogo, { currencyInfo: UNI_CURRENCY_INFO }));
        expect(tree).toMatchSnapshot();
    });
    it('renders a currency logo with network logo', () => {
        const tree = render(_jsx(CurrencyLogo, { currencyInfo: ARBITRUM_DAI_CURRENCY_INFO }));
        expect(tree).toMatchSnapshot();
    });
    describe('network logo', () => {
        it('is rendered by default', () => {
            const { queryByTestId } = renderWithProviders(_jsx(CurrencyLogo, { currencyInfo: arbitrumDaiCurrencyInfo() }));
            const networkLogo = queryByTestId('network-logo');
            expect(networkLogo).toBeTruthy();
        });
        it('is rendered if hideNetworkLogo is false', () => {
            const { queryByTestId } = renderWithProviders(_jsx(CurrencyLogo, { currencyInfo: arbitrumDaiCurrencyInfo(), hideNetworkLogo: false }));
            const networkLogo = queryByTestId('network-logo');
            expect(networkLogo).toBeTruthy();
        });
        it('is not rendered if hideNetworkLogo is true', () => {
            const { queryByTestId } = render(_jsx(CurrencyLogo, { hideNetworkLogo: true, currencyInfo: arbitrumDaiCurrencyInfo() }));
            const networkLogo = queryByTestId('network-logo');
            expect(networkLogo).toBeFalsy();
        });
    });
});
//# sourceMappingURL=CurrencyLogo.test.js.map