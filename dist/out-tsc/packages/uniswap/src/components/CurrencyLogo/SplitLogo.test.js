...
describe(SplitLogo, () => {
    it('renders without error', () => {
        const tree = render(_jsx(SplitLogo, { chainId: UniverseChainId.ArbitrumOne, inputCurrencyInfo: DAI_CURRENCY_INFO, outputCurrencyInfo: ETH_CURRENCY_INFO, size: 10 }));
        expect(tree).toMatchSnapshot();
    });
    describe('input currency logo', () => {
        it('renders input currency logo when inputCurrencyInfo is specified', () => {
            const { getByTestId } = render(_jsx(SplitLogo, { chainId: UniverseChainId.ArbitrumOne, inputCurrencyInfo: daiCurrencyInfo(), outputCurrencyInfo: ethCurrencyInfo(), size: 10 }));
        ...
        it('renders input currency logo when inputCurrencyInfo is not specified', () => {
            const { getByTestId } = render(_jsx(SplitLogo, { chainId: UniverseChainId.ArbitrumOne, inputCurrencyInfo: null, outputCurrencyInfo: ethCurrencyInfo(), size: 10 }));
            const inputCurrencyLogo = getByTestId('input-currency-logo-container');
    ...
    describe('output currency logo', () => {
        ...
        it('renders output currency logo when outputCurrencyInfo is not specified', () => {
            const { getByTestId } = render(_jsx(SplitLogo, { chainId: UniverseChainId.ArbitrumOne, inputCurrencyInfo: daiCurrencyInfo(), outputCurrencyInfo: null, size: 10 }));
            const outputCurrencyLogo = getByTestId('output-currency-logo-container');
    ...
    describe('icon', () => {
        ...
        it('does not render icon when chainId is not specified', () => {
            const { queryByTestId } = render(_jsx(SplitLogo, { chainId: null, inputCurrencyInfo: daiCurrencyInfo(), outputCurrencyInfo: ethCurrencyInfo(), size: 10 }));
            const icon = queryByTestId('network-logo');
            expect(icon).toBeFalsy();
        });
    });
});
//# sourceMappingURL=SplitLogo.test.js.map