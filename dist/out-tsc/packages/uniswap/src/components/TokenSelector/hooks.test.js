...
import { SAMPLE_SEED_ADDRESS_1, arbitrumDaiCurrencyInfo, daiToken, ethCurrencyInfo, ethToken, portfolio, portfolioBalance, portfolioBalances, token, tokenBalance, tokenProject, usdcArbitrumToken, usdcBaseToken, usdcCurrencyInfo, usdcToken, } from 'uniswap/src/test/fixtures';
...
const favoriteCurrencyIds = favoriteTokens.map((t) => { var _a; return buildCurrencyId((_a = fromGraphQLChain(t.chain)) !== null && _a !== void 0 ? _a : UniverseChainId.Mainnet, t.address); });
...
describe(usePopularTokensOptions, () => {
    ...
    it.each(cases)('$test', async ({ input, output }) => {
        const { resolvers } = queryResolvers(Object.fromEntries(Object.entries(input).map(([name, resolver]) => [name, queryResolver(resolver)])));
...
describe(useCommonTokensOptionsWithFallback, () => {
    ...
    it.each(cases)('$test', async ({ input: { chainFilter = null, ...resolverResults }, output }) => {
        const { resolvers } = queryResolvers(Object.fromEntries(Object.entries(resolverResults).map(([name, resolver]) => [name, queryResolver(resolver)])));
...
describe(useFavoriteTokensOptions, () => {
    ...
    const cases = [
        ...
        },
        {
            ...
            output: {
                data: expect.toIncludeSameMembers(favoriteTokenBalances.map((balance) => portfolioBalance({ fromBalance: balance }))),
                error: undefined,
            },
        },
        {
