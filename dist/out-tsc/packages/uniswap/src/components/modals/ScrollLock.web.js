...
export function initializeScrollWatcher() {
    if (isInitialized) {
        logger.error(new Error('`ScrollWatcher` already initialized. You should only call `initializeScrollWatcher` once.'), {
...
export function updateScrollLock({ isModalOpen }) {
    if (!isInitialized) {
...
export function useUpdateScrollLock({ isModalOpen }) {
    useEffect(() => {
        updateScrollLock({ isModalOpen });
        return () => updateScrollLock({ isModalOpen: false });
    }, [isModalOpen]);
}
//# sourceMappingURL=ScrollLock.web.js.map