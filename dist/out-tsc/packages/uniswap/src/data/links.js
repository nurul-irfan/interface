...
export const getCustomGraphqlHttpLink = (endpoint) => createHttpLink({
    uri: endpoint.url,
    headers: {
        'Content-Type': 'application/json',
...
export const getGraphqlHttpLink = () => createHttpLink({
...
export function sample(cb, rate) {
    if (Math.random() < rate) {
        cb();
    }
}
export function getErrorLink(graphqlErrorSamplingRate = APOLLO_GRAPHQL_ERROR_SAMPLING_RATE, networkErrorSamplingRate = APOLLO_NETWORK_ERROR_SAMPLING_RATE) {
    ...
    const errorLink = onError(({ graphQLErrors, networkError, operation }) => {
        if (graphQLErrors) {
            ...
            graphQLErrors.forEach(({ message, locations, path }) => {
                sample(() => logger.error(`GraphQL ${operationName} error: ${message}`, {
                    tags: {
                        file: 'data/links',
        ...
        if (networkError && !isMobileApp) {
            sample(() => logger.error(networkError, { tags: { file: 'data/links', function: 'getErrorLink' } }), networkErrorSamplingRate);
        }
    });
    return errorLink;
}
export function getPerformanceLink(sendAnalyticsEvent, samplingRate = APOLLO_PERFORMANCE_SAMPLING_RATE) {
    return new ApolloLink((operation, forward) => {
        const startTime = Date.now();
        return forward(operation).map((data) => {
            const duration = (Date.now() - startTime).toString();
            const dataSize = JSON.stringify(data).length.toString();
            sample(() => sendAnalyticsEvent({
                dataSize,
                duration,
