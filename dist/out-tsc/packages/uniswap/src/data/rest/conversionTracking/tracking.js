import ms from 'ms';
import { DEFAULT_HEADERS, GOOGLE_CONVERSION_URL, PERSONAL3_CONVERSION_URL, REDDIT_CONVERSION_URL, TWITTER_CONVERSION_URL, } from 'uniswap/src/data/rest/conversionTracking/constants';
import { PlatformIdType, RequestType } from 'uniswap/src/data/rest/conversionTracking/types';
import { addJitter, hashAddress } from 'uniswap/src/data/rest/conversionTracking/utils';
const buildTwitterProxyRequest = ({ lead, address, eventId, eventName, }) => ({
...
const buildPersona3ProxyRequest = ({ lead, address, eventId, eventName, }) => ({
    requestType: eventName,
    identifier: hashAddress(address),
...
const buildRedditProxyRequest = ({ lead, address, eventId, eventName, }) => ({
    requestType: eventName,
    identifier: hashAddress(address),
...
const buildGoogleProxyRequest = ({ lead, address, eventId, eventName, }) => ({
    requestType: eventName,
    identifier: hashAddress(address),
...
export const buildProxyRequest = (args) => {
    const { lead } = args;
    switch (lead.type) {
