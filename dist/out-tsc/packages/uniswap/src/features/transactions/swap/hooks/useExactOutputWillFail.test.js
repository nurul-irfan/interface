...
describe('useExactOutputWillFail', () => {
    const createToken = (buyFeeBps, sellFeeBps) => buildCurrency({
        chainId: 1,
    ...
    const createCurrencyInfo = (currency) => {
        return {
            currency,
    ...
    it('returns false for all flags when no fees are present', () => {
        const { result } = renderHook(() => useExactOutputWillFail({
            currencies: {
...
//# sourceMappingURL=useExactOutputWillFail.test.js.map