...
import { getRelevantTokenWarningSeverity, getShouldDisplayTokenWarningCard, } from 'uniswap/src/features/transactions/TransactionDetails/utils';
import { TransactionModalFooterContainer, TransactionModalInnerContainer, } from 'uniswap/src/features/transactions/TransactionModal/TransactionModal';
...
export function SwapReviewScreen(props) {
    ...
    const { derivedSwapInfo, exactCurrencyField: ctxExactCurrencyField, focusOnCurrencyField, isSubmitting, updateSwapForm, isFiatMode, } = useSwapFormContext();
    ...
    const { chainId, currencies, currencyAmounts, currencyAmountsUSDValue, txId, wrapType, trade: { trade, indicativeTrade }, // TODO(WEB-5823): rm indicative trade usage from review screen
     } = derivedSwapInfo;
    ...
    const { onSwap, validSwap } = useMemo(() => {
        ...
        if (isValidSwap && (account === null || account === void 0 ? void 0 : account.type) === AccountType.SignerMnemonic) {
            return {
                onSwap: () => {
                    swapCallback({
                        account,
    ...
    const submitButtonDisabled = (!validSwap && !isWrap) || !!blockingWarning || newTradeRequiresAcceptance || isSubmitting || isTokenWarningBlocking;
    ...
    if (submissionError) {
        return (_jsx(SwapErrorScreen, { submissionError: submissionError, setSubmissionError: setSubmissionError, resubmitSwap: onSwapButtonClick, onClose: onPrev }));
    }
    return (_jsxs(_Fragment, { children: [_jsxs(TransactionModalInnerContainer, { bottomSheetViewStyles: bottomSheetViewStyles, fullscreen: false, children: [(reviewScreenWarning === null || reviewScreenWarning === void 0 ? void 0 : reviewScreenWarning.warning.title) && (_jsx(WarningModal, { caption: reviewScreenWarning.warning.message, rejectText: blockingWarning ? undefined : t('common.button.cancel'), acknowledgeText: blockingWarning ? t('common.button.ok') : t('common.button.confirm'), isOpen: showWarningModal, modalName: ModalName.SwapWarning, severity: reviewScreenWarning.warning.severity, title: reviewScreenWarning.warning.title, onReject: onCancelWarning, onClose: onCloseWarning, onAcknowledge: onConfirmWarning })), _jsxs(Flex, { animation: "quick", opacity: hideContent ? 0 : 1, gap: "$spacing16", pt: isWeb ? '$spacing8' : undefined, children: [_jsx(TransactionAmountsReview, { acceptedDerivedSwapInfo: acceptedDerivedSwapInfo, newTradeRequiresAcceptance: newTradeRequiresAcceptance, onClose: onPrev }), showInterfaceReviewSteps ? (_jsx(ProgressIndicator, { currentStep: currentStep, steps: steps })) : isWrap ? (_jsx(TransactionDetails, { chainId: chainId, gasFee: gasFee, warning: reviewScreenWarning === null || reviewScreenWarning === void 0 ? void 0 : reviewScreenWarning.warning, txSimulationErrors: txSimulationErrors, onShowWarning: onShowWarning })) : (_jsx(SwapDetails, { acceptedDerivedSwapInfo: acceptedDerivedSwapInfo, autoSlippageTolerance: autoSlippageTolerance, customSlippageTolerance: customSlippageTolerance, derivedSwapInfo: derivedSwapInfo, feeOnTransferProps: feeOnTransferProps, tokenWarningProps: tokenWarningProps, tokenWarningChecked: tokenWarningChecked, setTokenWarningChecked: setTokenWarningChecked, gasFee: gasFee, newTradeRequiresAcceptance: newTradeRequiresAcceptance, uniswapXGasBreakdown: uniswapXGasBreakdown, warning: reviewScreenWarning === null || reviewScreenWarning === void 0 ? void 0 : reviewScreenWarning.warning, txSimulationErrors: txSimulationErrors, onAcceptTrade: onAcceptTrade, onShowWarning: onShowWarning }))] })] }), !showInterfaceReviewSteps && (_jsx(TransactionModalFooterContainer, { children: _jsxs(Flex, { row: true, gap: "$spacing8", children: [!isWeb && !showUniswapXSubmittingUI && (_jsx(Button, { icon: _jsx(BackArrow, {}), size: isShortMobileDevice ? 'medium' : 'large', theme: "tertiary", onPress: onPrev })), _jsx(SubmitSwapButton, { disabled: submitButtonDisabled, showUniswapXSubmittingUI: showUniswapXSubmittingUI, warning: reviewScreenWarning === null || reviewScreenWarning === void 0 ? void 0 : reviewScreenWarning.warning, onSubmit: onSwapButtonClick })] }) }))] }));
}
