...
export var AddressStringFormat;
(function (AddressStringFormat) {
    AddressStringFormat[AddressStringFormat["Lowercase"] = 0] = "Lowercase";
    AddressStringFormat[AddressStringFormat["Uppercase"] = 1] = "Uppercase";
    AddressStringFormat[AddressStringFormat["Shortened"] = 2] = "Shortened";
 ...
 */
export function getValidAddress(address, withChecksum = false, log = true) {
    try {
        if (!address) {
            return null;
        }
        ...
        if (log) {
            logger.warn('utils/addresses', 'getValidAddress', error === null || error === void 0 ? void 0 : error.message, {
                data: address,
 ...
 */
export function normalizeAddress(address, format) {
    switch (format) {
 ...
 */
export function sanitizeAddressText(address) {
    const zws = '\u{200b}'; // Zero-width space unicode
    return address === null || address === void 0 ? void 0 : address.replace('x', `x${zws}`);
}
