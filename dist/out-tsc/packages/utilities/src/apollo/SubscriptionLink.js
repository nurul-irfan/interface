    ...
    return new ApolloLink((operation, forward) => {
        ...
        return new Observable((observer) => {
            ...
            const interval = setInterval(async () => {
                const { data: { heartbeat: { success }, }, } = await client.mutate({ mutation: Heartbeat, variables: operation.variables });
                if (!success) {
...
export function createSubscriptionLink({ uri, region = '', // left blank for a custom domain name (eg realtime.gateway.uniswap.org)
token, }, client) {
