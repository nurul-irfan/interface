...
import { DappLogoWithTxStatus, DappLogoWithWCBadge, LogoWithTxStatus, } from 'wallet/src/components/CurrencyLogo/LogoWithTxStatus';
...
const nftLogoProps = createFixture()(() => ({
...
describe(LogoWithTxStatus, () => {
    it('renders without error', () => {
        const tree = render(_jsx(LogoWithTxStatus, { ...currencyLogoProps({
    ...
    describe('logo', () => {
        it('shows CurrencyLogo for currency', () => {
            const { queryByTestId } = render(_jsx(LogoWithTxStatus, { ...currencyLogoProps() }));
        ...
        it('shows NFTViewer in other cases', () => {
            const { queryByTestId } = render(_jsx(LogoWithTxStatus, { ...nftLogoProps() }));
            expect(queryByTestId('moonpay-logo')).toBeFalsy();
    ...
    describe('icon', () => {
        describe('transaction summary network logo', () => {
            it('shows network logo if chainId is specified and is not Mainnet', () => {
                const { queryByTestId } = render(_jsx(LogoWithTxStatus, { ...currencyLogoProps({ chainId: UniverseChainId.ArbitrumOne }) }));
                expect(queryByTestId('network-logo')).toBeTruthy();
            });
            it('does not show network logo if chainId is not specified', () => {
                const { queryByTestId } = render(_jsx(LogoWithTxStatus, { ...currencyLogoProps({ chainId: null }) }));
                expect(queryByTestId('network-logo')).toBeFalsy();
            });
            it('does not show network logo if chainId is Mainnet', () => {
                const { queryByTestId } = render(_jsx(LogoWithTxStatus, { ...currencyLogoProps({ chainId: UniverseChainId.Mainnet }) }));
                expect(queryByTestId('network-logo')).toBeFalsy();
            });
        });
        describe('transaction status icon when chainID is Mainnet', () => {
            ...
            const transactionWithoutIcons = Object.values(TransactionType).filter((txType) => !transactionWithIcons.includes(txType));
            const nftAssetTypesWithIcons = [AssetType.ERC721, AssetType.ERC1155];
            const nftAssetTypesWithoutIcons = Object.values(AssetType).filter((assetType) => !nftAssetTypesWithIcons.includes(assetType));
            for (const txType of transactionWithIcons) {
                it(`shows icon for ${txType}`, () => {
                    const { queryByTestId } = render(_jsx(LogoWithTxStatus, { ...currencyLogoProps({ chainId: UniverseChainId.Mainnet }), txType: txType }));
            ...
            for (const assetType of nftAssetTypesWithIcons) {
                it(`shows icon for NFTTrade if asset type ${assetType}`, () => {
                    const { queryByTestId } = render(_jsx(LogoWithTxStatus, { ...currencyLogoProps({ chainId: UniverseChainId.Mainnet }), assetType: assetType, txType: TransactionType.NFTTrade }));
                    expect(queryByTestId('status-icon')).toBeTruthy();
                });
            }
            for (const txType of transactionWithoutIcons) {
                it(`does not show icon for ${txType}`, () => {
                    ...
                    expect(consoleWarnMock).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.stringContaining('Could not find icon for transaction type:'), txType);
                });
            }
            for (const assetType of nftAssetTypesWithoutIcons) {
                it(`does not show icon for NFTTrade if asset type ${assetType}`, () => {
                    ...
                    const { queryByTestId } = render(_jsx(LogoWithTxStatus, { ...currencyLogoProps({ chainId: UniverseChainId.Mainnet }), assetType: assetType, txType: TransactionType.NFTTrade }));
                    expect(queryByTestId('status-icon')).toBeFalsy();
                    expect(consoleWarnMock).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.stringContaining('Could not find icon for transaction type:'), TransactionType.NFTTrade);
                });
            }
        });
    });
...
jest.mock('wallet/src/features/images/ImageUri', () => jest.requireActual('wallet/src/features/images/ImageUri.native.tsx'));
describe(DappLogoWithTxStatus, () => {
    ...
    describe('renders without error', () => {
        it('with dapp name', () => {
            const tree = render(_jsx(DappLogoWithTxStatus, { ...props }));
            expect(tree).toMatchSnapshot();
        });
    });
    describe('status icon', () => {
        const showedIconCases = [
        ...
        it.each(showedIconCases)('shows proper icon for %s event', (_, event, iconTestId) => {
            const { queryByTestId } = render(_jsx(DappLogoWithTxStatus, { ...props, event: event }));
            expect(queryByTestId(iconTestId)).toBeTruthy();
        });
        const hiddenIconCases = [
        ...
        it('does not render an icon if there is no event', () => {
            const { queryByTestId } = render(_jsx(DappLogoWithTxStatus, { ...props }));
            expect(queryByTestId('icon-approve')).toBeFalsy();
            expect(queryByTestId('icon-alert')).toBeFalsy();
            expect(queryByTestId('network-logo')).toBeFalsy();
        });
    ...
    describe('dapp image', () => {
        ...
        it('renders fallback image if dappImageUrl is not provided', () => {
            const { queryByTestId } = render(_jsx(DappLogoWithTxStatus, { ...props, dappImageUrl: undefined }));
...
describe(DappLogoWithWCBadge, () => {
    ...
    it('renders without error', () => {
        const tree = render(_jsx(DappLogoWithWCBadge, { ...props }));
        expect(tree).toMatchSnapshot();
    });
    describe('dapp image', () => {
        it('renders dapp icon placeholder if dappImageUrl is not provided', () => {
            const { queryByTestId } = render(_jsx(DappLogoWithWCBadge, { ...props, dappImageUrl: undefined }));
            expect(queryByTestId('dapp-image')).toBeFalsy();
            expect(queryByTestId('dapp-icon-placeholder')).toBeTruthy();
        });
        it('renders dapp image if dappImageUrl is provided', () => {
            const { queryByTestId } = render(_jsx(DappLogoWithWCBadge, { ...props }));
            expect(queryByTestId('dapp-image')).toBeTruthy();
    ...
    describe('network logo', () => {
        ...
        it('renders wallet connect logo if chain is Mainnet', () => {
            const { queryByTestId } = render(_jsx(DappLogoWithWCBadge, { ...props, chainId: UniverseChainId.Mainnet }));
            expect(queryByTestId('network-logo')).toBeFalsy();
            expect(queryByTestId('wallet-connect-logo')).toBeTruthy();
        });
    });
});
//# sourceMappingURL=LogoWithTxStatus.test.js.map