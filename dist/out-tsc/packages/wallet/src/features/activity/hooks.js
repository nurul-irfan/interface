...
import { useFeedTransactionListQuery, useTransactionListQuery, } from 'uniswap/src/data/graphql/uniswap-data-api/__generated__/types-and-hooks';
...
import { isLoadingItem, isSectionHeader } from 'wallet/src/features/activity/utils';
import { formatTransactionsByDate, parseDataResponseToFeedTransactionDetails, parseDataResponseToTransactionDetails, } from 'wallet/src/features/transactions/history/utils';
import { useMergeLocalAndRemoteTransactions } from 'wallet/src/features/transactions/hooks';
import { useAccounts } from 'wallet/src/features/wallet/hooks';
const LOADING_ITEM = (index) => ({ itemType: 'LOADING', id: index });
const LOADING_DATA = [LOADING_ITEM(1), LOADING_ITEM(2), LOADING_ITEM(3), LOADING_ITEM(4)];
export function useFormattedTransactionDataForFeed(addresses, hideSpamTokens) {
    var _a, _b;
    const { gqlChains } = useEnabledChains();
    const { refetch, networkStatus, loading: requestLoading, data, error: requestError, } = useFeedTransactionListQuery({
        variables: { addresses, chains: gqlChains },
        notifyOnNetworkStatusChange: true,
        // TODO: determine how often to poll for feed - currently slow
        pollInterval: PollingInterval.Slow,
    });
    const keyExtractor = useCallback((info) => {
    ...
    const { pending, last24hTransactionList, priorByMonthTransactionList } = useMemo(() => formatTransactionsByDate(transactions, localizedDayjs), [transactions, localizedDayjs]);
    const hasTransactions = transactions && transactions.length > 0;
    const hasData = !!((_b = (_a = data === null || data === void 0 ? void 0 : data.portfolios) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.assetActivities);
    ...
    const sectionData = useMemo(() => {
        ...
        return [
            ...
            ...Object.keys(priorByMonthTransactionList).reduce((accum, month) => {
...
export function useFormattedTransactionDataForActivity({ address, hideSpamTokens, pageSize, }) {
    var _a, _b, _c;
    const { gqlChains } = useEnabledChains();
    const { refetch, networkStatus, loading: requestLoading, data, error: requestError, } = useTransactionListQuery({
    ...
    const { pending, last24hTransactionList, priorByMonthTransactionList } = useMemo(() => formatTransactionsByDate(transactions, localizedDayjs), [transactions, localizedDayjs]);
    const hasTransactions = transactions && transactions.length > 0;
    const hasData = !!((_c = (_b = (_a = data === null || data === void 0 ? void 0 : data.portfolios) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.assetActivities) === null || _c === void 0 ? void 0 : _c.length);
    const isLoading = isNonPollingRequestInFlight(networkStatus);
    const isError = usePersistedError(requestLoading, requestError);
    // show loading if no data and fetching, or refetching when there is error (for UX when "retry" is clicked).
    const showLoading = (!hasData && isLoading) || (Boolean(isError) && networkStatus === NetworkStatus.refetch);
    ...
    const memoizedSectionDataRef = useRef(undefined);
    ...
    const memoizedSectionData = useMemo(() => {
        ...
        return sectionData.map((newItem) => {
            ...
            const oldItem = (_a = memoizedSectionDataRef.current) === null || _a === void 0 ? void 0 : _a.find((_oldItem) => newItemKey === keyExtractor(_oldItem));
