...
import { appRatingFeedbackProvidedMsSelector, appRatingPromptedMsSelector, appRatingProvidedMsSelector, } from 'wallet/src/features/wallet/selectors';
...
export const hasConsecutiveRecentSwapsSelector = createSelector([selectTransactions, (state) => { var _a; return (_a = state.wallet.appRatingPromptedMs) !== null && _a !== void 0 ? _a : 0; }], (transactions, appRatingPromptedMs) => {
    var _a;
    const swapTxs = [];
    ...
    const mostRecentSwapTime = (_a = recentSwaps[0]) === null || _a === void 0 ? void 0 : _a.addedTime;
    const mostRecentSwapLessThanMinAgo = Boolean(mostRecentSwapTime && Date.now() - mostRecentSwapTime < ONE_MINUTE_MS);
    return (swapTxs.length >= NUM_CONSECUTIVE_SWAPS &&
 ...
 */
export const appRatingStateSelector = createSelector([
    ...
    const hasPassedPromptDelay = !hasProvidedFeedback && hasPrompted && Date.now() - appRatingPromptedMs > MIN_PROMPT_REMINDER_MS;
    // Check if enough time has passed since last feedback
    const hasPassedFeedbackDelay = hasProvidedFeedback && Date.now() - appRatingFeedbackProvidedMs > MIN_FEEDBACK_REMINDER_MS;
