 ...
 */
export function getAdjustedGasFeeDetails(request, currentGasFeeParams, adjustmentFactor) {
    // Txn needs to be submitted with legacy gas params
    if (request.gasPrice) {
        const currentGasPrice = 'gasPrice' in currentGasFeeParams ? currentGasFeeParams.gasPrice : currentGasFeeParams.maxFeePerGas;
        return {
            type: FeeType.LEGACY,
            params: {
    ...
    if (request.maxFeePerGas && request.maxPriorityFeePerGas) {
        const currentMaxFeePerGas = 'maxFeePerGas' in currentGasFeeParams ? currentGasFeeParams.maxFeePerGas : currentGasFeeParams.gasPrice;
        const currentMaxPriorityFeePerGas = 'maxFeePerGas' in currentGasFeeParams ? currentGasFeeParams.maxPriorityFeePerGas : currentGasFeeParams.gasPrice;
        return {
            type: FeeType.EIP1559,
            params: {
                maxFeePerGas: multiplyByFactor(request.maxFeePerGas, currentMaxFeePerGas, adjustmentFactor),
                maxPriorityFeePerGas: multiplyByFactor(request.maxPriorityFeePerGas, currentMaxPriorityFeePerGas, adjustmentFactor),
            },
        };
    }
    throw determineError(request, currentGasFeeParams);
}
function determineError(request, currentGasFeeParams) {
    ...
    if (transactionMissingGasInfo) {
        if (isEIP1559Params) {
            ...
            return new Error('Transaction is missing gas values, but currentGasFeeParams were provided for a legacy transaction.');
        }
    }
    ...
    if (missingGasParams) {
        if (isEIP1559Transaction) {
            return new Error('currentGasFeeParams is missing gas fee parameters. Required: maxFeePerGas and maxPriorityFeePerGas for EIP-1559 transactions.');
        }
        else {
    ...
    if (EIP1559RequestMissingGasParams) {
        return new Error('Transaction request specifies EIP-1559 gas values, but currentGasFeeParams lacks corresponding EIP-1559 parameters.');
    }
    ...
    if (legacyRequestMissingGasParams) {
        return new Error('Transaction request specifies Legacy gasPrice, but currentGasFeeParams lacks a corresponding gasPrice.');
    }
...
function multiplyByFactor(value, minValue, adjustmentFactor) {
    const baseValue = BigNumberMax(BigNumber.from(value), BigNumber.from(minValue !== null && minValue !== void 0 ? minValue : 0));
    return Math.floor(baseValue.toNumber() * adjustmentFactor).toString();
}
//# sourceMappingURL=adjustGasFee.js.map