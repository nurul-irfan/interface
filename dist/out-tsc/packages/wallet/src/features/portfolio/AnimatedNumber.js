...
const RollNumber = ({ digit, nextColor, index, chars, commonPrefixLength, shouldFadeDecimals, currency, }) => {
...
const Char = ({ index, chars, currency, charsSizes, nextColor, commonPrefixLength, shouldFadeDecimals, }) => {
    ...
    return (_jsx(Animated.View, { style: { height: DIGIT_HEIGHT }, children: _jsx(Animated.View, { style: [
                ...
                animatedLeft,
            ], children: _jsx(RollNumber, { chars: chars, commonPrefixLength: commonPrefixLength, currency: currency, digit: chars[index], index: index, nextColor: nextColor, shouldFadeDecimals: shouldFadeDecimals }) }) }));
};
...
const ReanimatedNumber = ({ balance, currency, value, loading = false, loadingPlaceholderText, colorIndicationDuration, shouldFadeDecimals, warmLoading, }) => {
    ...
    return (_jsx(Animated.View, { style: scaleWraper, testID: TestID.PortfolioBalance, children: _jsxs(Flex, { row: true, alignItems: "flex-start", backgroundColor: "$surface1", borderRadius: "$rounded4", width: MAX_DEVICE_WIDTH, children: [_jsx(TopAndBottomGradient, {}), _jsx(Shine, { disabled: !warmLoading, children: _jsx(AnimatedFlex, { row: true, width: MAX_DEVICE_WIDTH, children: chars.map((_, index) => (_jsx(Char, { chars: chars, charsSizes: charsSizes, commonPrefixLength: commonPrefixLength, currency: currency, index: index, nextColor: nextColor, shouldFadeDecimals: shouldFadeDecimals }, index === 0 ? `$_sign_${colors.neutral1.val}` : `$_number_${chars.length - index}`))) }) }), _jsx(Animated.Text, { allowFontScaling: false, style: [AnimatedFontStyles.invisible, AnimatedFontStyles.fontStyle], onLayout: fitBalanceOnLayout, children: value })] }) }));
};
