...
export const PortfolioBalance = memo(function _PortfolioBalance({ owner }) {
    ...
    const shouldFadePortfolioDecimals = (currency === FiatCurrency.UnitedStatesDollar || currency === FiatCurrency.Euro) && currencyComponents.symbolAtFront;
    return (_jsxs(Flex, { gap: "$spacing4", children: [_jsx(AnimatedNumber, { balance: balanceUSD, colorIndicationDuration: BALANCE_CHANGE_INDICATION_DURATION, loading: isLoading, loadingPlaceholderText: "000000.00", shouldFadeDecimals: shouldFadePortfolioDecimals, value: totalBalance, warmLoading: isWarmLoading }), _jsx(Shine, { disabled: !isWarmLoading, children: _jsx(RelativeChange, { absoluteChange: absoluteChange, arrowSize: "$icon.16", change: percentChange, loading: isLoading, negativeChangeColor: isWarmLoading ? '$neutral2' : '$statusCritical', positiveChangeColor: isWarmLoading ? '$neutral2' : '$statusSuccess', variant: "body3" }) })] }));
});
//# sourceMappingURL=PortfolioBalance.js.map