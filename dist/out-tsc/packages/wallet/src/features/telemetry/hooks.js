...
import { selectAllowAnalytics, selectLastBalancesReport, selectLastBalancesReportValue, selectLastHeartbeat, selectWalletIsFunded, } from 'wallet/src/features/telemetry/selectors';
import { recordBalancesReport, recordHeartbeat, recordWalletFunded, shouldReportBalances, } from 'wallet/src/features/telemetry/slice';
import { useAccounts } from 'wallet/src/features/wallet/hooks';
export function useLastBalancesReporter() {
    ...
    const valueModifiers = usePortfolioValueModifiers(account === null || account === void 0 ? void 0 : account.address);
    const portfolioBalancesQuery = usePortfolioBalancesQuery({
        fetchPolicy: 'cache-only',
        variables: (account === null || account === void 0 ? void 0 : account.address) ? { ownerAddress: account.address, chains: gqlChains, valueModifiers } : undefined,
    });
    const totalBalancesUsdPerChain = useTotalBalancesUsdPerChain(portfolioBalancesQuery);
    const signerAccountValues = useMemo(() => {
        var _a;
        const valuesUnfiltered = (_a = data === null || data === void 0 ? void 0 : data.portfolios) === null || _a === void 0 ? void 0 : _a.map((p) => { var _a; return (_a = p === null || p === void 0 ? void 0 : p.tokensTotalDenominatedValue) === null || _a === void 0 ? void 0 : _a.value; }).filter((v) => v !== undefined);
        if (valuesUnfiltered === undefined) {
            return [];
        }
        return valuesUnfiltered;
    }, [data === null || data === void 0 ? void 0 : data.portfolios]);
    useEffect(() => {
        ...
        if (!walletIsFunded && sumOfFunds) {
            ...
            sendAppsFlyerEvent(MobileAppsFlyerEvents.WalletFunded, { sumOfFunds }).catch((error) => logger.debug('hooks', 'useLastBalancesReporter', error));
        }
    }, [dispatch, signerAccountValues, walletIsFunded]);
    const reporter = () => {
        if (shouldReportBalances(lastBalancesReport, lastBalancesReportValue, signerAccountAddresses, signerAccountValues)) {
            ...
            if (totalBalancesUsdPerChain && (account === null || account === void 0 ? void 0 : account.address)) {
...
export function useHeartbeatReporter() {
    const dispatch = useDispatch();
