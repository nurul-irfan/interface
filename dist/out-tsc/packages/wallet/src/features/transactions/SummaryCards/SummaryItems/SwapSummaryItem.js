...
export function SwapSummaryItem({ transaction, swapCallbacks, index, }) {
    ...
    const caption = useMemo(() => {
        ...
        const currencyAmount = getFormattedCurrencyAmount(inputCurrency, inputCurrencyAmountRaw, formatter, 
        ...
        const otherCurrencyAmount = getFormattedCurrencyAmount(outputCurrency, outputCurrencyAmountRaw, formatter, 
        //** isApproximateAmount - input value and confirmed amount are both exact so this should be false **//
        isConfirmedSwapTypeInfo(typeInfo) ? false : typeInfo.tradeType === TradeType.EXACT_INPUT);
        return `${currencyAmount}${getSymbolDisplayText(inputCurrency.symbol)} → ${otherCurrencyAmount}${getSymbolDisplayText(outputCurrency.symbol)}`;
    }, [inputCurrencyInfo, outputCurrencyInfo, formatter, typeInfo]);
    const icon = useMemo(() => (_jsx(SplitLogo, { chainId: transaction.chainId, inputCurrencyInfo: inputCurrencyInfo, outputCurrencyInfo: outputCurrencyInfo, size: TXN_HISTORY_ICON_SIZE })), [inputCurrencyInfo, outputCurrencyInfo, transaction.chainId]);
    return (_jsx(TransactionSummaryLayout, { caption: caption, icon: icon, index: index, transaction: transaction, onRetry: onRetry }));
}
//# sourceMappingURL=SwapSummaryItem.js.map