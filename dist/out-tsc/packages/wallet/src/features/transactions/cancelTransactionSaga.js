...
export function* attemptCancelTransaction(transaction, cancelRequest) {
...
function getPermit2Contract() {
    return new Contract(PERMIT2_ADDRESS, PERMIT2_ABI);
}
export async function getCancelOrderTxRequest(tx) {
    var _a;
    ...
    if (!orderHash) {
        ...
        const { encodedOrder } = (_a = (await getOrders([orderHash])).orders[0]) !== null && _a !== void 0 ? _a : {};
        if (!encodedOrder) {
            return undefined;
        }
...
function* cancelOrder(order, cancelRequest) {
...
//# sourceMappingURL=cancelTransactionSaga.js.map