...
export const useCanActiveAddressClaimUnitag = (address) => {
    const activeAddress = useActiveAccountAddressWithThrow();
    const targetAddress = address !== null && address !== void 0 ? address : activeAddress;
    ...
    useEffect(() => {
        ...
        refetch().catch((error) => logger.error(error, { tags: { file: 'unitags/hooks.ts', function: 'useCanActiveAddressClaimUnitag' } }));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [refetchUnitagsCounter]);
    return {
        canClaimUnitag: !isLoading && !!(data === null || data === void 0 ? void 0 : data.canClaim),
    };
};
export const useCanAddressClaimUnitag = (address, isUsernameChange) => {
    const { data: deviceId } = useAsyncData(getUniqueId);
    const { refetchUnitagsCounter } = useUnitagUpdater();
    const skip = !deviceId;
    const { isLoading, data, refetch } = useUnitagsClaimEligibilityQuery({
    ...
    useEffect(() => {
        ...
        refetch().catch((error) => logger.error(error, { tags: { file: 'unitags/hooks.ts', function: 'useCanAddressClaimUnitag' } }));
...
export const getUnitagFormatError = (unitag, t) => {
...
export const useCanClaimUnitagName = (unitag) => {
...
export const useClaimUnitag = () => {
    ...
    return async (claim, context, account) => {
        if (!claim.address) {
...
export const useAvatarUploadCredsWithRefresh = ({ unitag, account, signerManager, }) => {
    const [avatarUploadUrlLoading, setAvatarUploadUrlLoading] = useState(false);
    const [avatarUploadUrlResponse, setAvatarUploadUrlResponse] = useState();
    // Re-fetch the avatar upload pre-signed URL every 110 seconds to ensure it's always fresh
    useEffect(() => {
        const fetchAvatarUploadUrl = async () => {
            try {
...
export function useHasAnyAccountsWithUnitag() {
    var _a;
    const accounts = useSignerAccounts();
    const addresses = accounts.map((account) => account.address);
    const response = useUnitagsAddressesQuery({ params: { addresses } });
    return !!((_a = response.data) === null || _a === void 0 ? void 0 : _a.usernames.length);
}
