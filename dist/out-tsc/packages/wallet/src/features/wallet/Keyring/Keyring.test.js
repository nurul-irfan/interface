import { SAMPLE_PASSWORD, SAMPLE_SEED, SAMPLE_SEED_ADDRESS_1, SAMPLE_SEED_ADDRESS_10, SAMPLE_SEED_ADDRESS_2, SAMPLE_SEED_ADDRESS_3, SAMPLE_SEED_ADDRESS_4, SAMPLE_SEED_ADDRESS_5, SAMPLE_SEED_ADDRESS_6, SAMPLE_SEED_ADDRESS_7, SAMPLE_SEED_ADDRESS_8, SAMPLE_SEED_ADDRESS_9, } from 'uniswap/src/test/fixtures';
    ...
    return {
        ...
        callback) => {
            ...
            if (Array.isArray(key)) {
                return Promise.resolve(key.reduce((acc, k) => {
                    acc[k] = store[k];
                    return acc;
                }, {}));
            }
            if (typeof key === 'object') {
                return Promise.resolve(Object.keys(key).reduce((acc, k) => {
                    var _a;
                    const value = (_a = store[k]) !== null && _a !== void 0 ? _a : key[k];
                    acc[k] = value;
                    return acc;
                }, {}));
            }
...
jest.mock('./crypto', () => ({
    ...
    decrypt: jest.fn().mockImplementation(async ({ encryptionKey, ciphertext }) => {
