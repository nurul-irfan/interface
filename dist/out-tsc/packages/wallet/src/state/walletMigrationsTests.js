...
export function testActivatePendingAccounts(migration, prevSchema) {
    // all accounts active
    ...
    Object.values(SchemaAllActiveMigrated.wallet.accounts).forEach((acc) => {
    ...
    Object.values(SchemaNoAccountsMigrated.wallet.accounts).forEach((acc) => {
        expect(acc.pending).toBeUndefined();
    });
    ...
    Object.values(SchemaNoAccountsAndActiveAddressMigrated.wallet.accounts).forEach((acc) => {
        expect(acc.pending).toBeUndefined();
    });
    ...
    Object.values(SchemaNoActiveAddressMigrated.wallet.accounts).forEach((acc) => {
        expect(acc.pending).toBeUndefined();
    });
    ...
    Object.values(SchemaOnlyViewOnlyMigrated.wallet.accounts).forEach((acc) => {
        expect(acc.pending).toBeUndefined();
    });
    ...
    Object.values(SchemaMixedWithActiveViewOnlyMigrated.wallet.accounts).forEach((acc) => {
        expect(acc.pending).toBeUndefined();
    });
    ...
    Object.values(Schema7PendingAccountsMigrated.wallet.accounts).forEach((acc) => {
        expect(acc.pending).toBeUndefined();
    });
    ...
    const Schema8PendingAccountsActiveAddressInTheMiddleMigrated = migration(Schema8PendingAccountsActiveAddressInTheMiddle);
    ...
    Object.values(Schema8PendingAccountsActiveAddressInTheMiddleMigrated.wallet.accounts).forEach((acc) => {
        expect(acc.pending).toBeUndefined();
    });
    expect(Schema8PendingAccountsActiveAddressInTheMiddleMigrated.wallet.activeAccountAddress).toBe('0xTest5');
}
export function testAddedHapticSetting(migration, prevSchema) {
    const result = migration(prevSchema);
    expect(result.appearanceSettings.hapticsEnabled).toEqual(true);
}
export function testMovedUserSettings(migration, prevSchema) {
    const result = migration(prevSchema);
...
export function testRemoveHoldToSwap(migration, prevSchema) {
    const result = migration(prevSchema);
...
export function testAddCreatedOnboardingRedesignAccount(migration, prevSchema) {
    const result = migration(prevSchema);
    expect(result.behaviorHistory.createdOnboardingRedesignAccount).toBe(false);
}
export function testRemoveCreatedOnboardingRedesignAccount(migration, prevSchema) {
    const result = migration(prevSchema);
    expect(result.behaviorHistory.createdOnboardingRedesignAccount).toBe(undefined);
}
export function testMovedTokenWarnings(migration, prevSchema) {
...
export function testMovedLanguageSetting(migration, prevSchema) {
    const result = migration(prevSchema);
    expect(result.languageSettings).toEqual(undefined);
    expect(result.userSettings.currentLanguage).toEqual(Language.English);
}
export function testMovedCurrencySetting(migration, prevSchema) {
    const result = migration(prevSchema);
...
export function testUpdateExploreOrderByType(migration, prevSchema) {
    const result = migration(prevSchema);
    expect(result.wallet.settings.tokensOrderBy).toEqual(RankingType.Volume);
}
export function testUnchecksumDismissedTokenWarningKeys(migration, prevSchema) {
    const prevSchemaWithWarnings = {
        ...prevSchema,
        tokens: {
...
//# sourceMappingURL=walletMigrationsTests.js.map